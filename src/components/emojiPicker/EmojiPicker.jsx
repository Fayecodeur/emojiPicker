...
import { data as emojiList } from "../../data/data.jsx";
...
export function EmojiPicker(props, inputRef) {
  ...
  useEffect(() => {
    // Utilise le hook useEffect pour effectuer une action lors du rendu initial.
    window.addEventListener("click", (e) => {
      // Ajoute un écouteur d'événement au clic sur la fenêtre.
      if (!containerRef.current.contains(e.target)) {
        // Vérifie si le clic n'est pas à l'intérieur de containerRef.
  ...
  function handleSearch(e) {
    // Définit une fonction pour gérer la recherche d'emoji.
    ...
    if (!!value) {
      ...
      const search = emojiList.filter((emoji) => {
        // Filtre la liste des emojis en fonction de la recherche.
        return (
  ...
  function handleEmoji(emoji) {
    // Définit une fonction pour gérer le clic sur un emoji.
